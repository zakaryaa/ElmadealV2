class Salon < ActiveRecord::Base
  ...
  scope :appointment_ongoing_count, -> (id){ self.find(id).appointments.where(status: Appointment::VALIDE).where(status: Appointment::EN_COURS).where(status: Appointment::A_VALIDER).count}
